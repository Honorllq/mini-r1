...
> [!IMPORTANT]
> 训练和评测复用了 HumanEval `test` split 的同一组 164 道任务。下列 Pass@1 是 in-sample 配方对比，不是 held-out 泛化结果；泛化能力仍需独立代码基准验证。
...
| v4 | 1e-5 | 3 | 连续 | 62.20% | +10.37% | 30.7% | 5 | +17 | 📉 同任务回落 |
...
**结论**：v3 是当前同任务评测的甜蜜点。Epoch 2 reward 已接近 max；held-out 泛化仍待独立基准验证。

---

## v4: 第 3 epoch — 同任务性能回落
...
**结论**：第 3 epoch 后，同任务 Pass@1 回落 7.31%。这可能来自策略退化或对训练任务的过度优化，但当前实验不足以证明 held-out 泛化下降。
...
3. **小数据 (164 样本) 的同任务性能峰值：~2 epoch + LR 1e-5**
   v3 至 v4 单加 1 个 epoch 后 Pass@1 回落，说明继续优化训练 reward 未必改善任务表现。

4. **训练 reward 与评测 Pass@1 不严格单调**
   v4 训练 reward (1.053) > v3 (1.011)，但 Pass@1 反降。
   训练 reward 更高不等于同任务 Pass@1 更高；泛化需独立基准验证。
...
>    同任务 HumanEval Pass@1 从 51.83% → **69.51% (+17.68%)**；第 3 epoch 回落 7.31%，并识别出独立泛化评测的必要性。"
